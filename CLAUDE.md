...
Known gap left open deliberately: **skill naming**. The reference says a plugin skill's invocation name is its frontmatter `name`, with the directory basename only a fallback; skillmon keys `SkillId` on the basename. They agree for all 55 skills here, and `name_mismatch` (#27) surfaces divergence, so it is a no-op today — but changing it touches `SkillId`, usage attribution keys, and removal's id→dir mapping, so it is its own issue.
