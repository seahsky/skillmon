...
- Smaller: `message_usage` pruning and a byte-offset tail-reader landed together (issue #15, ADR 0024): a vanished checkpoint triggers a dir-scoped, conditional full rebuild (`wipe` + re-ingest, never a targeted delete), and `read_plan` tails only appended bytes. The rolling-24h usage window + toast budget of DESIGN.md UX #4 landed in issue #14.
