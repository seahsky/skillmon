...
**Next up** — the skill-source column and the removal ops (issues #24–#31, label `ready-for-agent`), designed in ADR 0026/0027 against a real 71-skill `~/.claude`:

- **Three shipped-today bugs, found while designing.** #24: 12 skills carry `disable-model-invocation: true`, so Claude Code drops them from the listing and charges zero always-on — skillmon doesn't parse the field, so those rows fall to the `Reconstructed` path and get billed a cost that doesn't exist. #25: symlink detection lstats only the skill dir, so it catches 20 of 66 managed skills — the dominant form on disk (a real dir whose `SKILL.md` is a symlink) reads as `is_symlink: false`. #26: the on-demand walk has no exclusions, so gstack's ceiling sweeps 704 MB of `node_modules`, a 60 MB `.git`, and 46 nested skills that are already their own rows — the "~216 MB bundled-file read" above understates it by ~5x.
