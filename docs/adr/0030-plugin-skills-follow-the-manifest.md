...
- **A declared path that climbs out of the plugin root is refused and warned.** The reference requires every manifest path to be relative to the root and start with `./`, and `Path::join` honors an absolute path by discarding the root entirely — so `"/Users/me/.claude/skills"` or `"../other-plugin/skills"` would silently credit another plugin's, or the user's own, skills to this one. Since the always-on total sums discovered skills (ADR 0003), that is the same skill in the headline twice. No manifest on disk does this; the guard is four lines and the failure it prevents is silent.
...
- **Naming is deliberately unchanged and is a known gap, and this decision adds the one layout where it bites.** The reference says a skill's invocation name comes from its frontmatter `name`, with the directory basename only as a fallback. skillmon keys `SkillId` on the directory basename for every skill kind. For all 55 skills on this machine the two agree, so this is a no-op for everything actually installed — but the root single-skill layout above is precisely the case the reference calls out as broken, since the basename there is the *install directory*, a version string (`1.2.0`, `unknown`) that changes on every update. Accepted rather than papered over: the row's footprint — the headline, and what skillmon is for — is right either way, and `DiscoveredSkill::name_mismatch` (issue #27) already exists to show both names rather than silently pick one, which is exactly this situation. `plugin_with_only_a_root_skill_md_is_loaded_as_a_single_skill` pins the wrong name and asserts the mismatch fires, so the gap is recorded where it bites rather than asserted around. Fixing it properly means keying `SkillId` on the declared name, which touches usage attribution keys, removal's id→directory mapping, and the documented invariant that `SkillId::name` *is* the directory name — its own issue (#41), not a rider on a visibility fix.
...
- **Treat `skills` as replacing the default scan**, per issue #33's own three-case model. Rejected against the reference: `skills` is documented to add. Indistinguishable on today's disk, wrong for a plugin that ships `skills/` *and* declares more — and the issue explicitly asked that the docs settle this rather than 11 manifests. It is worth naming what the documented model costs: under replacement, `mattpocock-skills`'s `skills/` is never scanned, so its 6 category dirs never produce a warning and `ChildDirs` would not need to exist at all. Additive semantics walk back into that directory, and classifying what is found there is the price of being right about the plugin that ships both.
